...
In this chapter things will get very interesting because we are going to set up our authentication mechanism. In my opinion it's one of the most interesting chapters. We will introduce a lot of new terms and you will end with a simple but powerful authentication system. Don’t panic, we will get there.
...
> JWT is an open standard defined in RFC 7519. It allows the secure exchange of tokens between several parties. - https://wikipedia.org/wiki/JSON_Web_Token_Web_Token[Wikipedia]
...
This has many advantages such as sending information in the token's payload. For example, we may choose to integrate user's information into the _payload_.
...
. If the password corresponds to the _hash_, JSON containing the _token_ generated with the class `JsonWebToken` is returned. Otherwise, an empty response is returned with an `unauthorized` header
...
We can now run our new tests which, missing the `current_user` implementation, we would expect to fail - which they do. So let's implement the code so that it can be passed:
...
Let's start by changing the _should update user_ test.
...
Failure:
Expected response to be a <403: forbidden>, but was a <200: OK>
